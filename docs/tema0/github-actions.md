...
\| \`pull\_request\` | Se abre, actualiza o cierra una PR |

\| \`schedule\` | Un temporizador (como un cron job) |

\| \`workflow\_dispatch\` | El usuario lo activa manualmente desde la web |
...
\| **\*\*Event\*\*** | El disparador que pone en marcha el workflow (\`push\`, \`pull\_request\`...). |
...
  pull\_request:                        # En cualquier PR hacia main
...
    Fíjate en el \`@v7\` de \`actions/checkout\@v7\`. Indica la versión mayor de la Action. En estos ejemplos utilizamos la **versión mayor estable recomendada** de cada Action oficial (por ejemplo, \`@v7\`), que recibe correcciones compatibles dentro de esa rama. Evita referencias como \`@main\`: apuntan al desarrollo más reciente y podrían introducir cambios inesperados sin que hayas modificado tu workflow.

    Desde junio de 2026 GitHub Actions utiliza **Node.js 24** como runtime predeterminado para las Actions JavaScript. Por eso conviene mantener actualizadas las Actions oficiales y, si utilizas runners autohospedados, mantener también actualizado el software del runner.
...
    Error: JAVA\_HOME is not set and 'java' is not in PATH
...
3\. Dale un nombre en mayúsculas (convenio): \`MAVEN\_PASSWORD\`, \`DEPLOY\_TOKEN\`...
...
    MAVEN\_USERNAME: ${{ secrets.MAVEN\_USERNAME }}   # se sustituye en tiempo de ejecución

    MAVEN\_PASSWORD: ${{ secrets.MAVEN\_PASSWORD }}   # nunca aparece en el log
...
    - Un **\*\*workflow\*\*** se activa con un **\*\*event\*\*** (\`push\`, \`pull\_request\`...), contiene **\*\*jobs\*\*** que corren en un **\*\*runner\*\***, y cada job tiene **\*\*steps\*\*** que ejecutan comandos o **\*\*Actions\*\*** del Marketplace.
