...
- **`docs/seguridad.md`** (la tabla de políticas de seguridad, un fichero compartido con PSP): a día de hoy no incluye las rutas de `Review` que construiste en el Tema 3 — sí incluye ya, en cambio, la ruta de WebSocket que PSP ha añadido en su propio Tema 4. Un documento de seguridad que no se actualiza al mismo ritmo que el código deja de ser de fiar.
- **El procedimiento almacenado `ajustar_precio_estudio`** (Actividad 1.4): no tiene ninguna documentación de qué hace o por qué existe fuera del propio código SQL — ni un comentario, ni ninguna referencia en el proyecto que explique la lógica de negocio del descuento que aplica.
